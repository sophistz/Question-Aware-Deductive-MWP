...
from transformers import PreTrainedTokenizerFast, BertTokenizerFast, BertTokenizer, RobertaTokenizer, \
...
UniFeature = collections.namedtuple('UniFeature',
                                    'input_ids attention_mask token_type_ids variable_indexs_start variable_indexs_end num_variables variable_index_mask labels label_height_mask question_start')
...
class UniversalDataset(Dataset):
    ...
    def read_math23k_file(self, file: Union[str, None],
        ...
        for obj in tqdm(data, desc='Tokenization', total=len(data)):
            ...
            try:
                if self.use_incremental_labeling:
                    res, _ = compute_value_for_incremental_equations(labels, obj["num_list"], self.constant_num,
                                                                     uni_labels=self.uni_labels,
                                                                     constant_values=self.constant_values)
                else:
                    res = compute_value(labels, obj["num_list"], self.constant_num, uni_labels=self.uni_labels,
                                        constant_values=self.constant_values)
            except:
        ...
        logger.info(
            f", total number instances: {len(self._features)} (before filter: {len(data)}), max num steps: {max_num_steps}")
        ...
        logger.debug(
            f"average operation number: {avg_eq_num}, total: {equation_layer_num}, counter: {equation_layer_num_count}")
        avg_sent_len = sent_len_all * 1.0 / len(self._features)
        logger.debug(f"average sentence length: {avg_sent_len}, total: {sent_len_all}")
        logger.debug(
            f"variable number avg: {var_num_all * 1.0 / len(self._features)}, total: {var_num_all}, counter:{var_num_count}")
    ...
    def get_label_ids_updated(self, equation_layers: List, add_replacement: bool) -> Union[List[List[int]], None]:
        ...
        for l_idx, layer in enumerate(equation_layers):
            ...
            if left_var != "#" and (not left_var.startswith("m_")):
                ...
                left_var_idx = -1
            right_var_idx = (ord(right_var) - ord('a') + num_constant) if self.constant2id is None or (
                        right_var not in self.constant2id) else self.constant2id[right_var]
            # try:
            assert right_var_idx >= 0
            # except:
            ...
            if left_var_idx <= right_var_idx:
                ...
                if (op in ["+", "*"]):
                    ...
                    # assert not op.endswith("_rev")
                    op_idx = self.uni_labels.index(op + "_rev") if not op.endswith("_rev") else self.uni_labels.index(
                        op[:-4])
                    label_ids.append([right_var_idx, left_var_idx, op_idx, is_stop])
        return label_ids
                ...
                if left_var.startswith("m_") and (not right_var.startswith("m_")):
                    ...
                    if left_var_idx >= right_var_idx:  ##left larger means left m_idx smaller
                        ...
                        if left_var_idx > right_var_idx and (op not in ["+", "*"]):
                            op_idx = self.uni_labels.index(op + "_rev") if not op.endswith(
                                "_rev") else self.uni_labels.index(op[:-4])
                        label_ids.append([right_var_idx, left_var_idx, op_idx, is_stop])
                    else:
                        # left < right
                        if (op in ["+", "*"]):
    ...
    def collate_function(self, batch: List[UniFeature]):
        ...
        for i, feature in enumerate(batch):
            ...
            batch[i] = UniFeature(input_ids=np.asarray(input_ids),
                                  ...
                                  # zyy
                                  question_start=feature.question_start
                                  )
...
def main_for_mawps():
    ...
    constants = ['12.0', '1.0', '7.0', '60.0', '2.0', '5.0', '100.0', '8.0', '0.1', '0.5', '0.01', '25.0', '4.0', '3.0',
                 '0.25']
    ...
    test_dataset = UniversalDataset(file="../../data/mawps-single/mawps_test_nodup.json", tokenizer=tokenizer,
    ...
    train_dataset = UniversalDataset(file="../../data/mawps-single/mawps_train_nodup.json", tokenizer=tokenizer,
                                     uni_labels=uni_labels,
                                     constant2id=constant2id, constant_values=constant_values,
                                     add_replacement=add_replacement,
                                     use_incremental_labeling=use_incremental_labeling,
                                     pretrained_model_name=pretrained_language_moel)
    validation_dataset = UniversalDataset(file="../../data/mawps-single/mawps_valid_nodup.json", tokenizer=tokenizer,
                                          uni_labels=uni_labels,
                                          constant2id=constant2id, constant_values=constant_values,
                                          add_replacement=add_replacement,
                                          use_incremental_labeling=use_incremental_labeling,
...
def main_for_svamp():
    ...
    constants = ['1.0', '0.1', '3.0', '5.0', '0.5', '12.0', '4.0', '60.0', '25.0', '0.01', '0.05', '2.0', '10.0',
                 '0.25', '8.0', '7.0', '100.0']
    ...
    pretrained_language_moel = 'roberta-base'  ## bert-base-cased, roberta-base, bert-base-multilingual-cased, xlm-roberta-base
    tokenizer = class_name_2_tokenizer[pretrained_language_moel].from_pretrained(pretrained_language_moel)
    UniversalDataset(file="../../data/mawps_asdiv-a_svamp/testset_nodup.json", tokenizer=tokenizer,
                     uni_labels=uni_labels,
    ...
    UniversalDataset(file="../../data/mawps_asdiv-a_svamp/trainset_nodup.json", tokenizer=tokenizer,
                     uni_labels=uni_labels,
                     constant2id=constant2id, constant_values=constant_values, add_replacement=add_replacement,
                     use_incremental_labeling=use_incremental_labeling, pretrained_model_name=pretrained_language_moel)
...
def main_for_mathqa():
    ...
    constants = ['100.0', '1.0', '2.0', '3.0', '4.0', '10.0', '1000.0', '60.0', '0.5', '3600.0', '12.0', '0.2778',
                 '3.1416', '3.6',
    ...
    UniversalDataset(file="../../data/MathQA/mathqa_test_nodup_our_filtered.json", tokenizer=tokenizer,
                     uni_labels=uni_labels,
                     constant2id=constant2id, constant_values=constant_values, add_replacement=add_replacement,
                     data_max_height=data_max_height,
                     use_incremental_labeling=use_incremental_labeling, pretrained_model_name=pretrained_language_moel)
    UniversalDataset(file="../../data/MathQA/mathqa_dev_nodup_our_filtered.json", tokenizer=tokenizer,
                     uni_labels=uni_labels,
                     constant2id=constant2id, constant_values=constant_values, add_replacement=add_replacement,
                     data_max_height=data_max_height,
                     use_incremental_labeling=use_incremental_labeling, pretrained_model_name=pretrained_language_moel)
    UniversalDataset(file="../../data/MathQA/mathqa_train_nodup_our_filtered.json", tokenizer=tokenizer,
                     uni_labels=uni_labels,
                     constant2id=constant2id, constant_values=constant_values, add_replacement=add_replacement,
                     data_max_height=data_max_height,
                     use_incremental_labeling=use_incremental_labeling, pretrained_model_name=pretrained_language_moel)
